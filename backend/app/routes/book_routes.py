...
# Import necessary FastAPI and SQLAlchemy components.
from fastapi import APIRouter, HTTPException, Depends
from sqlalchemy.orm import Session
from app.database import Book, Transaction, get_db
from app.auth_utils import get_current_user
from app.database import User
from datetime import datetime
...
# PATCH route to update book status.
@router.patch("/books/{isbn}/status")
def update_book_status(
    isbn: str,
    status: str,
...
# GET route to fetch book status by ISBN.
@router.get("/books/{isbn}")
def get_book_status(isbn: str, db: Session = Depends(get_db)):
    book = db.query(Book).filter(Book.isbn == isbn).first()
    if not book:
        raise HTTPException(status_code=404, detail="Book not found")
    return {
